"""Case study CS01 — falsify-eval against the BEIR NFCorpus benchmark.
...
Reproduction:
    ...
    # ~5 minutes on M1 16GB. Outputs results/cs01_results.json.

Hardware notes:
- Loads NFCorpus dev split: 323 queries, 3,633 docs. ~10 MB total.
...
Reference scores (from BEIR paper, Thakur et al. 2021, Table 2):
    BM25 nDCG@10 on NFCorpus      ≈ 0.325
    DPR  nDCG@10 on NFCorpus      ≈ 0.183
...
# ----------------------------------------------------------------------------
# 1. Load NFCorpus from HuggingFace's BEIR mirror — locked, hashed, reproducible
# ----------------------------------------------------------------------------

def load_nfcorpus():
    """Downloads NFCorpus dev split via the BEIR HuggingFace mirror.
    ...
    print("[load] downloading BeIR/nfcorpus (corpus + queries + qrels)...")
...
def make_recall_at_k_top1(k=5):
    ...
    like NFCorpus where nDCG-style metrics collapse the null distributions."""
...
def main():
    ...
    print("=" * 72)
    print("falsify-eval CS01 — NFCorpus (BEIR public benchmark)")
    print("=" * 72)

    corpus, queries, qrels, manifest = load_nfcorpus()
    ...
    out_path = RESULTS_DIR / "cs01_results.json"
